...
**Banners** — three `<a class="banner">` cards (Resume, GitHub, LinkedIn) sit in a `div.banners` flexbox row. Icons are inline SVGs (zero network requests). Real URLs are loaded from `config.js` (gitignored — copy from `config.example.js`); banners that still have `href="#"` are blocked from scrolling to the top via `e.preventDefault()`.
